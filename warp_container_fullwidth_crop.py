...
def main():
    parser = argparse.ArgumentParser(description='Warp container to full width, crop to remove black border, and optionally blend')
    ...
    debug_dir = base / 'debug_warp_fullwidth_crop'
    if args.debug:
    ...
    for idx, img in enumerate(imgs):
        ...
        # Crop lại theo mask đã warp để loại bỏ viền đen
        cropped_img, cropped_mask = crop_to_mask(warped, warped_mask)
        warped_imgs.append(cropped_img)
        if args.debug:
            img_draw = img.copy()
            ...
            cv2.imwrite(str(debug_dir / f'cropped_{idx:03d}.png'), cropped_img)
    print(f'Warped and cropped {len(warped_imgs)} images.')
    # Blend các ảnh đã crop (resize về cùng size)
    if warped_imgs:
        h0, w0 = warped_imgs[0].shape[:2]
        ...
        out_path = base / f'container_warped_fullwidth_crop_{args.start}_{args.end}.jpg'
