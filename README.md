...
Overview
--------
This repository contains a small image-processing pipeline to detect, trim, align, rectify and stitch photographs of container side faces into panoramas. The goal is reliable, straight panoramas of container sides suitable for visual inspection or downstream analysis.

High-level stages
-----------------
1. Detection & corner estimation
   - Scripts: `detect_yolo_corners.py`, `detect_yolo_corners_sides.py` (side-specific variants)
   - Purpose: detect container faces and estimate bounding corners. Outputs corner text files per image.

2. Trimming / corner updates
...
3. Corner refinement & visualization
   - Scripts: `corner_refinement.py`, `corner_refinement_sides.py`, `visualize_corners.py`
   - Purpose: refine corner locations (gradient-based) and produce visual verification images with corners overlaid.

4. Alignment / warp to common canvas
...
5. (Optional) Rectify to rectangle
...
7. Post-processing / cleanup
   - Scripts: `cleanup_detections.py`, `generate_masks_from_corners.py`, `pad_cropped_sides.py`
...
---------------------------------------
1. Detect corners for raw side images (example):

   python3 detect_yolo_corners.py --dir raw_images --out corners_dir

2. Trim black borders and update corner files:
...
4. (Optional) Rectify to rectangles and write masks:
...
- Add a short example script `run_pipeline.sh` that runs the whole sequence for a given index range.
- Add a small visual diff tool to compare seam modes and seam widths across a crop region.

---
Generated on request. If you want the README extended with parameter descriptions or a JSON manifest of paths, tell me which format you prefer.
