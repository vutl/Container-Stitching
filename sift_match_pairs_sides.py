...
def _estimate_transform(pts1: np.ndarray, pts2: np.ndarray, mode: str) -> np.ndarray:
    mode = (mode or 'translation').lower()
    if mode in ('affine', 'aff'):
...
def run_pair(a_path: Path, b_path: Path, a_mask: Path = None, b_mask: Path = None, out: Path = None, transform: str = 'translation'):
    ...
    # Also compute transform from filtered matches and create a simple stitch
    stitched_out = None
    if len(filt_matches) >= 3:
        ...
        H = _estimate_transform(pts1, pts2, transform)
        if H is None:
            print('Transform estimation failed; falling back to identity')
            H = np.eye(3, dtype=np.float32)
    ...
    print('Saved stitch', stitched_out, 'mode=', transform, 'matches=', len(filt_matches))
