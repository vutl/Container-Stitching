#!/usr/bin/env python3
"""
Side-face rectification: copy of `rectify_to_rectangle.py` tuned for side
images aligned into `aligned_sides/`. Outputs rectified images to
`aligned_rectified_sides/` by default.
"""
...
def main():
    ...
    ap.add_argument('--dir', required=True, help='Input directory containing aligned images (default: aligned_sides)')
    ap.add_argument('--indices', required=True, help='Frame indices (e.g. "210-261")')
    ap.add_argument('--image-suffix', default='_aligned.jpg')
    ap.add_argument('--corners-suffix', default='_aligned_corners.txt')
    ap.add_argument('--out', default='aligned_rectified_sides')
