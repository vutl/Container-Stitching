...
def _preprocess_for_phasecorr(gray: np.ndarray) -> np.ndarray:
    g = gray.astype(np.float32)
    # High-pass: subtract blurred to reduce illumination mismatch
    blur = cv2.GaussianBlur(g, (0, 0), sigmaX=3, sigmaY=3)
    hp = g - blur
    # Normalize to unit variance to improve stability
    std = hp.std() if hp.size else 1.0
    if std < 1e-3:
        std = 1.0
    hp = hp / std
    return hp
...
def estimate_translation_phasecorr(src: np.ndarray, tar: np.ndarray) -> Optional[np.ndarray]:
    """Estimate pure translation using phase correlation on preprocessed grayscale images.

    Returns 3x3 H or None on failure.
    """
    try:
        gs = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
        gt = cv2.cvtColor(tar, cv2.COLOR_BGR2GRAY)
    except Exception:
        return None
    # Resize to common size for PC (speed + stability)
    h = min(gs.shape[0], gt.shape[0])
    w = min(gs.shape[1], gt.shape[1])
    if h < 64 or w < 64:
        return None
    # Use central crop to avoid black borders
    def center_crop(img, hh, ww):
        y0 = max(0, (img.shape[0] - hh) // 2)
        x0 = max(0, (img.shape[1] - ww) // 2)
        return img[y0:y0+hh, x0:x0+ww]

    hh = int(h * 0.8)
    ww = int(w * 0.8)
    gs_c = center_crop(gs, hh, ww)
    gt_c = center_crop(gt, hh, ww)

    # Optionally downscale to ~512 width
    target_w = 512
    scale = 1.0
    if ww > target_w:
        scale = target_w / float(ww)
        new_size = (int(ww * scale), int(hh * scale))
        gs_c = cv2.resize(gs_c, new_size, interpolation=cv2.INTER_AREA)
        gt_c = cv2.resize(gt_c, new_size, interpolation=cv2.INTER_AREA)

    a = _preprocess_for_phasecorr(gs_c)
    b = _preprocess_for_phasecorr(gt_c)
    ...
    try:
        ...
        return None
    dx_f, dy_f = float(shift[0]) / max(scale, 1e-6), float(shift[1]) / max(scale, 1e-6)
    if not np.isfinite([dx_f, dy_f]).all():
        return None
    # Guardrails: absurd shifts are likely wrong
    if abs(dx_f) > max(src.shape[1], tar.shape[1]) or abs(dy_f) > max(src.shape[0], tar.shape[0]):
        return None
    H = np.eye(3, dtype=np.float32)
    # phaseCorrelate returns shift to align src to tar; use as translation
    H[0, 2] = dx_f
    H[1, 2] = dy_f
    return H
    ...
    if np.any(overlap):
        if blend_mode == 'none':
            ...
            if blend_mode_local == 'seam':
                ...
                # Build blended subregion using soft alpha mask; apply simple exposure compensation first
                alpha = fmask[..., None]
                subA_f = subA.astype(np.float32)
                subB_f = subB.astype(np.float32)
                # exposure match subB to subA in overlap region
                ov = mask_sub
                if np.count_nonzero(ov) > 200:
                    meanA = subA_f[ov].mean(axis=0)
                    meanB = subB_f[ov].mean(axis=0)
                    stdB = subB_f[ov].std(axis=0)
                    stdB = np.maximum(stdB, 1e-3)
                    scale = np.clip(meanA / stdB * (subB_f[ov].std(axis=0) / subA_f[ov].std(axis=0) if subA_f[ov].std(axis=0).all() else 1.0), 0.6, 1.6)
                    shift = meanA - scale * meanB
                    subB_f = np.clip(subB_f * scale[None, None, :] + shift[None, None, :], 0, 255)
                ...
                w_old = 1.0 - w_new
                # Simple exposure match on the overlap for consistency
                subA_f = subA.astype(np.float32)
                subB_f = subB.astype(np.float32)
                ov = mask_sub
                if np.count_nonzero(ov) > 200:
                    meanA = subA_f[ov].mean(axis=0)
                    meanB = subB_f[ov].mean(axis=0)
                    stdA = np.maximum(subA_f[ov].std(axis=0), 1e-3)
                    stdB = np.maximum(subB_f[ov].std(axis=0), 1e-3)
                    scale = np.clip(stdA / stdB, 0.6, 1.6)
                    shift = meanA - scale * meanB
                    subB_f = np.clip(subB_f * scale[None, None, :] + shift[None, None, :], 0, 255)

                blended = subA_f * w_old[..., None] + subB_f * w_new[..., None]
                sub_out = out[y0:y1, x0:x1]
            ...
            w_new = dist_new / denom
            w_old = 1.0 - w_new
            # Exposure compensation for wide overlaps
            A = moved.astype(np.float32)
            B = warped_new.astype(np.float32)
            ov = overlap
            if np.count_nonzero(ov) > 5000:
                meanA = A[ov].mean(axis=0)
                meanB = B[ov].mean(axis=0)
                stdA = np.maximum(A[ov].std(axis=0), 1e-3)
                stdB = np.maximum(B[ov].std(axis=0), 1e-3)
                scale = np.clip(stdA / stdB, 0.6, 1.6)
                shift = meanA - scale * meanB
                B = np.clip(B * scale[None, None, :] + shift[None, None, :], 0, 255)

            blended = A * w_old[..., None] + B * w_new[..., None]

            # Optional two-level multiband blend for very wide overlaps
            if np.count_nonzero(overlap) > 12000:
                sigma = 6
                A_blur = cv2.GaussianBlur(A, (0, 0), sigma)
                B_blur = cv2.GaussianBlur(B, (0, 0), sigma)
                A_det = A - A_blur
                B_det = B - B_blur
                blended = A_blur * w_old[..., None] + B_blur * w_new[..., None] 
                blended += A_det * w_old[..., None] + B_det * w_new[..., None]
...
def stitch_incremental_with_corners(image_paths: List[Path],
    ...
    for k in range(1, len(idxs)):
        ...
        if H is None:
            H = estimate_translation_phasecorr(cur_img, mosaic)

        # Fallback to ECC if still None
        if H is None:
