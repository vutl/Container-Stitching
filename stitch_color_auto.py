...
def sample_hsv_range(img, roi_frac=0.28, tol_h=15, tol_s=60, tol_v=60):
    """Sample a central ROI and return (lower, upper) HSV bounds (0..179,0..255,0..255).
    Handles hue wrap-around."""
    h, w = img.shape[:2]
    ...
    # also sample from a looser container ROI if available (try to prefer higher saturation)
    h_img, w_img = img.shape[:2]
    rmask = container_roi(img)
    # collect HSV where roi mask is nonzero
    hsv_full = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    coords = np.where(rmask > 0)
    if coords[0].size > 50:
        samples = hsv_full[coords]
        med_roi = np.median(samples.reshape(-1, 3), axis=0).astype(int)
        h_r, s_r, v_r = int(med_roi[0]), int(med_roi[1]), int(med_roi[2])
    else:
        h_r, s_r, v_r = h_c, s_c, v_c

    # pick the sample with larger saturation (prefer more distinct color)
    if s_r > s_c:
        h_med, s_med, v_med = h_r, s_r, v_r
    else:
...
def hybrid_mask(prev, cur, lower, upper):
    """Union of color mask and bbox-from-edges for more robust coverage, with cleanup."""
    ...
    # fill small holes by connected components
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats((union > 0).astype(np.uint8), connectivity=8)
    out = np.zeros_like(union)
    # keep components bigger than threshold (pixels)
    min_px = max(1000, int(0.002 * union.size))
...
def stitch_incremental_color(image_paths, suffix, debug=False, use_bbox=False, force_rect=False):
